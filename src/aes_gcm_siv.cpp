    ...
    std::span<byte> ciphertext)
{
    ...
    std::span<const __m128i> pt{
        reinterpret_cast<const __m128i*>(plaintext.data()),
        plaintext.size() / 16
    };
    
    std::span<const __m128i> aad{
        reinterpret_cast<const __m128i*>(additional_data.data()),
    ...
    std::span<byte> plaintext)
{
    ...
    const __m128i* ctptr = reinterpret_cast<const __m128i*>(ciphertext.data());

    __m128i nonce_128 = ctptr[0];

    std::span<const __m128i> ct{
        ctptr + 1,
        ciphertext.size() / 16
    };
    ...
    std::span<char> ciphertext)
{
    return aes_gcm_siv_enc(
        std::as_bytes(plaintext),
        std::as_bytes(additional_data),
        std::as_bytes(key),
